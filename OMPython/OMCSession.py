...
class OMSessionABC(metaclass=OMSessionMeta):
    ...
    def set_timeout(self, timeout: Optional[float] = None) -> float:
        ...
        if timeout is not None:
            if timeout <= 0.0:
                raise OMSessionException(f"Invalid timeout value: {timeout}s!")
    ...
    * OMCSessionWSL
    ...
    def sendExpression(self, expr: str, parsed: bool = True) -> Any:
        ...
        while next(loop):
            ...
            raise OMSessionException(f"No connection with OMC (timeout={self._timeout:.2f}s).")
...
class OMCSessionLocal(OMCSessionABC):
    ...
    def _omc_port_get(self) -> str:
        ...
        while next(loop):
            ...
            raise OMSessionException(f"OMC Server did not start (timeout={self._timeout:.2f}s, "
...
class OMCSessionDockerABC(OMCSessionABC, metaclass=abc.ABCMeta):
    ...
    def _docker_process_get(self, docker_cid: str) -> Optional[DockerPopen]:
        ...
        while next(loop):
            ...
            raise OMSessionException(f"Docker based OMC Server did not start (timeout={self._timeout:.2f}s).")
...
class OMCSessionWSL(OMCSessionABC):
    ...
    def _omc_port_get(self) -> str:
        ...
        while next(loop):
            ...
            raise OMSessionException(f"WSL based OMC Server did not start (timeout={self._timeout:2f}s, "
                                     f"logfile={repr(self._omc_logfile)}).")
